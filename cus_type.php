<?php
    session_start();
        ...
        ?>
                ...
                <?php
                echo '<meta http-equiv="refresh" content="0;url=cus_category.php" />';
        }
?>
<!DOCTYPE html>
<html lang="en">
...
<head>
  ...
  <title>Lianga eShopping | Product Categories</title>
                                 ...
                                 <div class="row">
                                    <?php
                                        ...
                                        if(isset($_POST['search_button'])) {
                                            ...
                                            $query3 = mysqli_query($mysql, "select *,max(sold) as max_sold, p.image as image from product p, monitoring m, types t where (p.name like '%$product%' or p.description like '%$product%' or p.brand like '%$product%' or t.types like '%product%')
                                                        and (p.product_id=m.product_id and p.type_id=t.type_id) and t.type_id='$type_id' group by p.product_id order by m.date_time desc;");
                                            ...
                                            $query3 = mysqli_query($mysql, "select *, max(sold) as max_sold,p.image as image from product p, monitoring m, types t where (p.product_id=m.product_id and
                                                        p.type_id=t.type_id) and t.type_id='$type_id' group by p.product_id order by m.date_time desc;");
                                            while($row3 = mysqli_fetch_array($query3)) {
                                                ?>
                                                <div class="col-sm-4 col-md-2 col-6 margin_bottom_30">
                                                   <div class="column">
