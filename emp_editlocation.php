<?php
    session_start();
        ...
        if(isset($_POST['upd_location'])){
            $query1 = mysqli_query($mysql, "update location set pickup='$_POST[upd_pickup]', charge='$_POST[upd_charge]', travel_time='$_POST[upd_travel_time]' where location_id='$_POST[upd_location_id]';");
        ?>
                  ...
                  <div class="login_form">
                     <form class="fill_up" action="" method="post" role="form">
                        <fieldset>
                           <?php
                                $id = $_GET['id'];
                                $query1 = mysqli_query($mysql, "select * from location where location_id='$id';");
                                while($row1 = mysqli_fetch_array($query1)){
                                    ?>

                           <div class="field">
                              <label class="label_field">Location ID</label>
                              <input type="text" name="upd_location_id" readonly value="<?php echo $row1['location_id'];?>" />

                           </div>
                           <div class="field">
                              <label class="label_field">Location</label>
                              <input type="text" name="upd_pickup" value="<?php echo $row1['pickup'];?>" required />
                           </div>
                           <div class="field">
                              <label class="label_field">Charge</label>
                              <input type="number" min=1 name="upd_charge" value="<?php echo $row1['charge'];?>" required />
                           </div>
                           <div class="field">
                              <label class="label_field">Total Minutes to Travel</label>
                              <input type="number" min=1 name="upd_travel_time" value="<?php echo $row1['travel_time'];?>" required />
                           </div>
                           <div align="center">
                              <input style="margin:20px 40px 20px 40px;" name="upd_location" type="submit" class="btn btn-primary" value="Save" />
                              <a style="margin:20px 40px 20px 40px;;" href="emp_location.php" class="btn btn-danger">Cancel</a>
                           </div>
                           <?php
                                }
                           ?>
