<?php
    session_start();
        ...
        if(isset($_POST['upd_order'])){
            $query1 = mysqli_query($mysql, "update orders set quantity='$_POST[upd_quantity]',total='$_POST[upd_total]',charge='$_POST[upd_charge]',
                        sum_total='$_POST[upd_sum_total]',location='$_POST[upd_location]' where order_id='$_POST[upd_order_id]';");
        ?>
                <script>alert("Data successfully updated.");</script>
                <?php
                echo '<meta http-equiv="refresh" content="0;url=adm_order.php" />';
        }
?>
...
<head>
  ...
  <title>Lianga eShopping | Admin - Order</title>
                  ...
                  <div class="login_form">
                     <form class="fill_up" action="" method="post" role="form">
                        <fieldset>
                           <?php
                                $id = $_GET['id'];
                                $query1 = mysqli_query($mysql, "select * from orders where order_id='$id';");
                                while($row1 = mysqli_fetch_array($query1)){
                                    ?>

                           <div class="field">
                              <label class="label_field">Order ID</label>
                              <input type="text" name="upd_order_id" readonly value="<?php echo $row1['order_id'];?>" />
                           </div>
                           <div class="field">
                              <label class="label_field">Quantity</label>
                              <input type="text" name="upd_quantity" value="<?php echo $row1['quantity'];?>" required />
                           </div>
                           <div class="field">
                              <label class="label_field">Total</label>
                              <input type="text" name="upd_total" value="<?php echo $row1['total'];?>" required />
                           </div>
                           <div class="field">
                              <label class="label_field">Charge</label>
                              <input type="text" name="upd_charge" value="<?php echo $row1['charge'];?>" required />
                           </div>
                           <div class="field">
                              <label class="label_field">Sum Total</label>
                              <input type="text" name="upd_sum_total" value="<?php echo $row1['sum_total'];?>" required />
                           </div>
                           <div class="field">
                              <label class="label_field">Location</label>
                              <input type="text" name="upd_location" value="<?php echo $row1['location'];?>" required />
                           </div>
                           <div class="field">
                           ...
                           <div align="center">
                              <input style="margin:20px 40px 20px 40px;" name="upd_order" type="submit" class="btn btn-primary" value="Save" />
                              <a style="margin:20px 40px 20px 40px;;" href="adm_cart.php" class="btn btn-danger">Cancel</a>
                           </div>
                           <?php
                                }
                           ?>
