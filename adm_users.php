<?php
    session_start();
        ...
        if(isset($_POST['add_users'])) {
            ...
            $query2 = mysqli_query($mysql, "insert into users values('$_POST[user_id]','$_POST[username]','$_POST[password]','admin',NOW());");
            $query4 = mysqli_query($mysql, "update count set user='$_POST[user_count]';");
        ?>
                    <script type="text/javascript">
                        alert("Data successfully added to database");
                    </script>
                <?php
                echo '<meta http-equiv="refresh" content="0;url=adm_users.php" />';
...
<head>
  ...
  <title>Lianga eShopping | Admin - List of Users</title>
                              ...
                              <div class="table_section padding_infor_info">
                                 ...
                                 <div class="table-responsive-sm">
                                    <table class="table table-striped table-bordered" id="dataTables-example">
                                       ...
                                       <tbody>
                                           <?php
                                                $qry2 = mysqli_query($mysql, "select * from users;");
                                        		while($row2 = mysqli_fetch_array($qry2))
                                        		{
                                        		    $qry4 = mysqli_query($mysql, "select * from users u, users_employee ue, employee e where u.user_id=ue.user_id and ue.employee_id=e.employee_id and ue.user_id='$row2[user_id]';");
                                                     $num_row4 = mysqli_num_rows($qry4);
                                                     $data4 = mysqli_fetch_array($qry4);
                                                     $qry5 = mysqli_query($mysql,"select * from users u, users_customer uc, customer c where u.user_id=uc.user_id and uc.customer_id=c.customer_id and uc.user_id='$row2[user_id]';");
                                                     $num_row5 = mysqli_num_rows($qry5);
                                                     $data5 = mysqli_fetch_array($qry5);
                                                      $qry6 = mysqli_query($mysql,"select * from users u, users_supplier us, supplier s where u.user_id=us.user_id and us.supplier_id=s.supplier_id and us.user_id='$row2[user_id]';");
