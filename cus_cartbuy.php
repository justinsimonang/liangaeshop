<?php
    session_start();
        ...
        ?>
            ...
            $count1 = $row2['orders'] + $count;
            $qry2 = mysqli_query($mysql, "update count set orders='$count1';");

            $j = 0;
            foreach($_POST['order_id'] as $order_id) { $order_id1[$j] = $order_id; $j++; } $j = 0;
            foreach($_POST['quantity'] as $quantity) { $quantity1[$j] = $quantity; $j++; } $j = 0;
            foreach($_POST['sub_total'] as $sub_total) { $sub_total1[$j] = $sub_total; $j++; } $j = 0;
            foreach($_POST['charge'] as $charge) { $charge1[$j] = $charge; $j++; } $j = 0;
            foreach($_POST['order_total'] as $order_total) { $order_total1[$j] = $order_total; $j++; } $j = 0;
            foreach($_POST['product_id'] as $prod_id) { $prod_id1[$j] = $prod_id;  $j++; } $j = 0;
            foreach($_POST['cart_id'] as $cart_id) { $cart_id1[$j] = $cart_id;  $j++; }

            for($i=0; $i<$count; $i++){
                $order_id = $order_id1[$i];
                $quantity = $quantity1[$i];
                $sub_total = $sub_total1[$i];
                $charge = $charge1[$i];
                $order_total = $order_total1[$i];
                $prod_id = $prod_id1[$i];
                $cart_id = $cart_id1[$i];
                $qry3 = mysqli_query($mysql, "insert into orders values('$order_id',NOW(),'$quantity','$sub_total','$charge','$order_total','For Processing - Waiting to accept order','$prod_id','$id_number','$location_id','');");
                $qry4 = mysqli_query($mysql, "insert into cart_orders values('$cart_id','$order_id','$id_number','$prod_id','$location_id');");
                $qry5 = mysqli_query($mysql, "update cart set status='checked-out' where cart_id='$cart_id';");
                }
                /*
            for($i=0; $i<$count; $i++){
                $order_id = $order_id1[$i];
        ...
        ?>
            ...
            if(!empty($_POST['ckb'])) {
            foreach($_POST['ckb'] as $values) {
                $fields = explode("|", $values);
                $cart_id = $fields[1];
                $query1=mysqli_query($mysql, "update cart set status='deleted' where cart_id='$cart_id';");
        }
    }
                ?>
...
<head>
  ...
  <title>Lianga eShopping | Cart Checkout</title>
                           ...
                           <div class="white_shd full margin_bottom_30">
                              ...
                              <div class="full gallery_section_inner padding_infor_info">
                                 ...
                                 <div class="row">
                                    ...
                                    <?php
                                        ...
                                        if(!empty($_POST['ckb'])) {
                                            foreach($_POST['ckb'] as $values) {
                                                ...
                                                <?php
                                                $query1=mysqli_query($mysql, "select *,c.quantity as cart_qty from cart c, product p where c.product_id=p.product_id and cart_id='$cart_id';");
                                                while($row1=mysqli_fetch_array($query1)) {
                                                    $count++;
                                                    //echo $count;
                                                    $sub_total = $row1['price'] * $row1['cart_qty'];
                                                    $order_total = $sub_total + $charge;

                                                    $qry2 = mysqli_query($mysql, "select * from count;");
                                                    $row2 = mysqli_fetch_array($qry2);
                                                    $count2 = $row2['orders'] + $count;
                                                    $add2 = 70000 + $count2;
                                                    $id2 = "ORDE-".$add2;
                                            ?>

                                                    <input type="hidden" name="order_id[]" value="<?php echo $id2;?>" />
                                                    <input type="hidden" name="quantity[]" value="<?php echo $row1['cart_qty'];?>" />
                                                    ...
                                                    <div class="col-md-1 col-2"><img class="zoom" style="margin: 0 10px 0 10px;" src="<?php echo "upload/".$row1['image'];?>"></div>
                                                    <div class="col-md-5 col-10"><?php echo $row1['description'];?></div>
                                                    <div class="col-md-1 col-6"><?php echo "₱".$row1['price'];?></div>
                                                    <div align="right" class="col-md-1 col-6"><?php echo $row1['cart_qty']." item(s)";?></div>
                                            ...
                                            <?php
                                                $merchandise_total += $sub_total;
                                                $delivery_total += $charge;
                                    ...
                                    ?>
                                    <input type="hidden" name="order_count" value="<?php echo $count; ?>" />
                                 </div>
                                 ...
                                 <div class="row">
                                     ...
                                     <div align="right" class="col-md-1 col-5"><?php echo "₱".$merchandise_total;?></div>
                                     <div class="col-md-9 col-12"></div>
                                     <div class="col-md-2 col-7">Delivery Fee Subtotal: &nbsp;</div>
                                     <div align="right" class="col-md-1 col-5"><?php echo "₱".$delivery_total;?></div>
                                     <div class="col-md-9 col-12"></div>
                                     <div class="col-md-2 col-7" style="color:#155660;"><b>Total Payment: &nbsp;</b></div>
                                     <div align="right" class="col-md-1 col-5" style="font-size:20px; color:#155660;"><b><?php echo "₱".$total_payment;?></b></div>
                                 </div>
                              </div>
                           </div>
                        </div>
    <header id="header" class="fixed-bottom d-flex align-items-center" style="background:#FFF; opacity:.90;">
        <div class="container d-flex justify-content-between">
          <div class="col-md-11 col-7" align="right" style="font-size:20px; color:#155660;">
            ...
            &nbsp;<b><div id=msg1 style="float:right;"><?php echo "₱".$total_payment;?></div></b>
          </div>
